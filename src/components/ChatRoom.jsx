...
import {
  ...
  limit, getDocs, onSnapshot, startAfter, serverTimestamp,
...
export default function ChatRoom({ userName }) {
      ...
      const replyPayload = capturedReply
        ? {
            replyTo: Object.fromEntries(
              Object.entries(capturedReply).filter(([, v]) => v !== undefined)
            ),
          }
        : {}
  ...
  const handleImageSelect = async (e) => {
    ...
    if (file.size > 10 * 1024 * 1024) { alert('Ảnh không được vượt quá 10MB!'); return }
  ...
  return (
    ...
    >
      ...
      <header className="topbar">
        ...
        <div className="topbar-right">
          ...
          <div className="user-badge">
            <div className="avatar" style={{ background: avatarColor }}>{getInitials(userName)}</div>
            <span>{userName}</span>
          </div>
        </div>
              ...
              <div className="online-panel-list">
                {onlineUsers.map((u) => (
                  <div key={u.userName} className="online-user-item">
                    <div className="avatar" style={{ background: getAvatarColor(u.userName), width: 30, height: 30, fontSize: 12, borderRadius: 10 }}>
      ...
      {/* ── Messages ── */}
      <main className="messages-wrap" ref={msgsWrapRef} onScroll={handleScroll}>
        {loading && (
        ...
        {!loading && firestoreError && (
          <div className="empty-state">
            ...
            <p style={{ color: 'var(--danger)', fontSize: 12, marginTop: 4 }}>Lỗi: <code>{firestoreError}</code></p>
            <p style={{ marginTop: 8, lineHeight: 1.8 }}>
              1. Đã tạo <b>Firestore Database</b> chưa?<br />
              2. Rules: <code>allow read, write: if true</code>
            </p>
            <button onClick={() => window.location.reload()} style={{ marginTop: 16, padding: '8px 20px', borderRadius: 10, background: 'var(--accent-gradient)', border: 'none', color: 'white', cursor: 'pointer', fontSize: 13, fontFamily: 'inherit' }}>
              🔄 Thử lại
            </button>
          </div>
        )}
        ...
        {!loading && hasMore && (
          <div style={{ display: 'flex', justifyContent: 'center', padding: '8px 0 4px' }}>
            <button className="load-more-btn" onClick={loadMore} disabled={loadingMore}>
              {loadingMore ? <><span className="upload-spinner" style={{ width: 12, height: 12 }} /> Đang tải...</> : '⬆ Tải tin nhắn cũ hơn'}
            </button>
          </div>
        )}
        ...
        {!loading && (() => {
          ...
          return allMsgs.map((msg, i) => {
            ...
            return (
              <div key={msg.id}>
                {showDate && <div className="date-sep"><span>{formatDate(msg.createdAt)}</span></div>}
                <Message
                  msg={msg}
                  isMe={isMe}
                  showMeta={showMeta}
        ...
        <div className="reply-preview">
          ...
          <div className="reply-preview-content">
            ...
            <span className="reply-preview-text">
              {replyTo.type === 'image' ? '🖼️ Hình ảnh' : replyTo.text}
            </span>
          </div>
      ...
      <footer className="input-bar">
        ...
        <button className="img-btn" onClick={() => fileInputRef.current?.click()} disabled={isBusy} title="Gửi ảnh" aria-label="Gửi ảnh">🖼️</button>
        ...
        <button className="send-btn" onClick={sendMessage} disabled={!text.trim() || isBusy} aria-label="Gửi tin nhắn">➤</button>
      </footer>
    </div>
  )
}
